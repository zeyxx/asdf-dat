...
function loadTokensFromStateFile(): TokenConfig[] {
  ...
  try {
    ...
    for (const t of stateData.tokens) {
      ...
      // CRITICAL FIX: Creator must be DAT Authority for all tokens
      // The creator_vault PDA is derived from ["creator-vault", creator]
      // All our tokens use datAuthority as creator (set during token creation via Pump.fun)
      ...
      tokens.push({
        ...
        creator: t.creator ? new PublicKey(t.creator) : datAuthority, // Use stored creator or fallback to datAuthority
        isRoot: t.isRoot === true,
        isToken2022: t.isToken2022 ?? false,
        mayhemMode: t.mayhemMode ?? false,
        poolType,
