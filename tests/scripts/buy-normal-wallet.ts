...
async function buyWithIDL(amountSol: number) {
  ...
  // Le creator est le DAT Authority (vérifié dans devnet-token-info.json)
  const creator = new PublicKey("6r5gW93qREotZ9gThTV7SAcekCRaBrua6e1YSxirfNDs");
  
  console.log(`Creator (DAT Authority): ${creator.toString()}`);
  ...
  // Paramètres
  const amountTokens = new BN(1_000_000_000); // 1 billion tokens
