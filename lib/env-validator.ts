...
export const AlertingEnvSchema = z.object({
  ...
  ALERT_ENABLED: z.enum(['true', 'false']).transform(v => v === 'true').default('true'),
...
export function validateAlertingEnv(): AlertingEnv {
  ...
  // Warn if alerting enabled but no webhook URL
  if (result.data.ALERT_ENABLED && !result.data.WEBHOOK_URL) {
    console.warn('Warning: ALERT_ENABLED=true but WEBHOOK_URL not set. Alerts will not be sent.');
  }
